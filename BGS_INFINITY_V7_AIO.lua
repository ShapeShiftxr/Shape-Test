...
║  Vollständig dynamisch — alle Daten live aus ReplicatedStorage              ║
╚══════════════════════════════════════════════════════════════════════════════╝
]]
...
local CFG = {
    ...
    AutoRift         = false,
    RiftDelay        = 0.0,
    RiftPriority     = {},         -- geordnete egg-priorität
    AutoOpenRift     = true,
...
--  BlowBubble() — kein separates SellBubble nötig (server-side)
--  Aber: SellBubble ist ein eigenes Remote für manuelles Sell
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
--  RIFT SYSTEM
--  Workspace.Rendered.Rifts enthält live Rift-Instanzen
...
local function doRiftTP(riftEntry)
    ...
    -- Hatche (alle ausgewählten eggs beim rift)
...
local lbl = {
    bpm      = T.Stats:CreateLabel("🫧 Bubbles/min: —"),
    epm      = T.Stats:CreateLabel("🥚 Eggs/min: —"),
    total_b  = T.Stats:CreateLabel("🫧 Bubbles gesamt: —"),
    total_e  = T.Stats:CreateLabel("🥚 Eggs gesamt: —"),
    uptime   = T.Stats:CreateLabel("⏱ Uptime: —"),
    coins    = T.Stats:CreateLabel("💰 Coins: —"),
    gems     = T.Stats:CreateLabel("💎 Gems: —"),
    tickets  = T.Stats:CreateLabel("🎟 Tickets: —"),
}

T.Stats:CreateSection("Rejoin")
T.Stats:CreateToggle({ Name="Auto Rejoin (zufällig)", CurrentValue=false, Flag="AutoRejoin",
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            lbl.bpm:Set("🫧 Bubbles/min: "..formatNum(bMin))
            lbl.epm:Set("🥚 Eggs/min: "..formatNum(eMin))
            lbl.total_b:Set("🫧 Bubbles gesamt: "..formatNum(State.BubblesBlown))
            lbl.total_e:Set("🥚 Eggs gesamt: "..formatNum(State.EggsHatched))
            lbl.uptime:Set("⏱ Uptime: "..up)

            -- Währungen aus LocalData
            ...
            if ld and ld.Get then
                ...
                if d and d.Currencies then
                    lbl.coins:Set("💰 Coins: "..formatNum(d.Currencies.Coins or 0))
                    lbl.gems:Set("💎 Gems: "..formatNum(d.Currencies.Gems or 0))
                    lbl.tickets:Set("🎟 Tickets: "..formatNum(d.Currencies.Tickets or 0))
                end
            end
        end)
    end
...
T.Eggs:CreateDropdown({
    Name="Egg auswählen (Dropdown)", Options=ALL_EGGS,
...
-- Verzögerung
T.Eggs:CreateSlider({ Name="TP Verzögerung (Sek.)", Range={0,2}, Increment=0.1, CurrentValue=0, Flag="RiftDelay",
...
-- Prioritätsliste
T.Eggs:CreateDropdown({
    Name="Prioritäts-Reihenfolge (oben = höchste)", Options=ALL_EGGS,
    CurrentOption={}, MultipleSelection=true, Flag="RiftPriority",
    Callback=function(v) CFG.RiftPriority=type(v)=="table" and v or {v} end,
})

T.Eggs:CreateToggle({ Name="Priorität aktiv", CurrentValue=false, Flag="PriorityActive",
...
T.Eggs:CreateToggle({ Name="Rift öffnen wenn in der Nähe", CurrentValue=true, Flag="AutoOpenRift",
...
-- Rift Spawn & Open
T.Eggs:CreateSection("Rift Spawnen & Öffnen")
T.Eggs:CreateButton({ Name="RiftSummon (×5)", Callback=function()
    task.spawn(function() for i=1,5 do fire("RiftSummon"); task.wait(0.3) end end)
    notify("Rift","RiftSummon ×5 ✓",2)
end })
T.Eggs:CreateButton({ Name="MoonSummon (×5)", Callback=function()
    task.spawn(function() for i=1,5 do fire("MoonSummon"); task.wait(0.3) end end)
    notify("Rift","MoonSummon ×5 ✓",2)
end })
...
-- Potions sortiert nach Priorität
local POT_PRIORITY = {
    -- Standard Lv7
...
-- Füge restliche Elixiere dynamisch hinzu
...
T.Potions:CreateButton({ Name="⚡ ALLE (Priorität: beste zuerst)", Callback=function()
    task.spawn(function()
        for _,p in ipairs(POT_PRIORITY) do
            fire("CraftPotion", p.name, p.level, true)
            task.wait(0.2)
        end
        notify("Potions","Alle aktiviert ✓",3)
    end)
end })

T.Potions:CreateButton({ Name="Spam ×10 (10x alle)", Callback=function()
    task.spawn(function()
        for rep=1,10 do
            for _,p in ipairs(POT_PRIORITY) do
                fire("CraftPotion", p.name, p.level, true)
        ...
        notify("Potions","Spam ×10 ✓",3)
    end)
end })

-- Beste Potions als einzelne Spam-Buttons
T.Potions:CreateSection("Top Potions — Spam")
...
for _, p in ipairs(topPotions) do
    ...
    T.Potions:CreateButton({ Name=label.." — Spam ×10", Callback=function()
        task.spawn(function()
            for i=1,10 do fire("CraftPotion",pn,pl,true); task.wait(0.1) end
        end)
        notify("Potion",label.." ×10 ✓",2)
    end })
    T.Potions:CreateButton({ Name=label.." — Spam ×100", Callback=function()
        task.spawn(function()
            for i=1,10 do
...
T.Potions:CreateDropdown({ Name="Elixier auswählen", Options=specialList,
...
T.Genie:CreateLabel("Rerollt alle 4s bis gewünschte Quest erscheint")
        ...
        if v then
            task.spawn(function()
                while CFG.AutoGenie do
                    ...
                    -- Wähle Quest wenn verfügbar (StartGenieQuest mit Index 1-3)
...
T.Genie:CreateButton({ Name="Quest 1 auswählen",   Callback=function() fire("StartGenieQuest",1) end })
T.Genie:CreateButton({ Name="Quest 2 auswählen",   Callback=function() fire("StartGenieQuest",2) end })
T.Genie:CreateButton({ Name="Quest 3 auswählen",   Callback=function() fire("StartGenieQuest",3) end })
...
T.Fishing:CreateSection("Ausrüstung")
...
-- Bait Dropdown mit Anzahl (live aus Inventar — Fallback aus Data)
local baitList = tableKeys(D.FishingBait)
T.Fishing:CreateDropdown({ Name="Köder (Equip)", Options=baitList,
...
T.Fishing:CreateButton({ Name="SellAllFish",                   Callback=function() fire("SellAllFish"); notify("Fishing","✓",2) end })
T.Fishing:CreateButton({ Name="ClaimAllFishingIndexRewards",   Callback=function() fire("ClaimAllFishingIndexRewards"); notify("Fishing","✓",2) end })
...
T.Mini:CreateSection("Minigames — Auto Loop")
...
for miniName, miniData in pairs(D.Minigames) do
    if type(miniData)=="table" then
        ...
        T.Mini:CreateButton({ Name="Finish (einmalig)", Callback=function()
            fire("FinishMinigame"); notify("Mini",mn.." ✓",2)
        end })
        ...
        T.Mini:CreateButton({ Name="Super Ticket + Finish", Callback=function()
            ...
            fire("FinishMinigame"); notify("Mini","Ticket + "..mn.." ✓",2)
        end })
...
T.Board:CreateLabel("Großer Dice wenn Ziel weit weg — kleinere wenn nah")
        ...
        if v then
            task.spawn(function()
                while CFG.AutoDice do
                    ...
                    -- (Position-Tracking würde serverseitig Board-State brauchen)
                    -- Daher: fire UseItem Golden Dice alle CFG.GoldenThreshold Züge
...
T.Board:CreateButton({ Name="×5 Dice Spam",       Callback=function()
...
T.Shops:CreateLabel("Kauft automatisch ausgewählte Shops leer")

T.Shops:CreateDropdown({ Name="Shops auswählen", Options=SHOP_LIST,
...
T.Shops:CreateButton({ Name="Buy Selected Shops", Callback=function()
    task.spawn(function()
        ...
        notify("Shop","Alle ausgewählten Shops ✓",3)
    end)
end })
...
T.Misc:CreateSection("Teleport — Areas")
...
T.Misc:CreateDropdown({ Name="Area auswählen", Options=areaList,
...
T.Misc:CreateDropdown({ Name="Spin Ticket auswählen", Options=SPIN_TYPES,
...
T.Misc:CreateDropdown({ Name="Gift Box auswählen", Options=GIFT_BOXES,
    CurrentOption={}, MultipleSelection=true, Flag="SelectedBoxes",
    Callback=function(v) CFG.SelectedBoxes=type(v)=="table" and v or {v} end })

T.Misc:CreateToggle({ Name="Auto Gift Box öffnen", CurrentValue=false, Flag="AutoGiftBox",
    Callback=function(v)
        CFG.AutoGiftBox=v
        if v then
            task.spawn(function()
...
T.Misc:CreateButton({ Name="Alle Codes einlösen", Callback=redeemAll })
...
T.Misc:CreateButton({ Name="Unlock All Hatching Zones", Callback=function()
    ...
    notify("Unlock","HatchZones ✓",3)
end })
T.Misc:CreateButton({ Name="ClaimAllPlaytime", Callback=function() fire("ClaimAllPlaytime"); notify("Misc","✓",2) end })
...
T.Webhook:CreateSlider({ Name="Min. Chance für Notify (%)", Range={0,5}, Increment=0.01, CurrentValue=0.1,
...
T.Webhook:CreateLabel("Webhook wird beim Hatch-Event ausgelöst")
...
task.defer(function()
    ...
    notify(
        ...
        ok
            and ("✅ "..#EGG_RIFTS.." Egg-Rifts | "..cnt(D.Potions).." Potions | "..#CODES.." Codes | "..#ALL_EGGS.." Eggs")
            or "⚠️ Remotes noch nicht bereit...",
        6
    )
    if not ok then
        task.spawn(function()
